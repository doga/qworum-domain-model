  ...
  writeTo(rdfDataset){
    ...
    try {
      ...
      for (let i = 0; i < this.emails.length; i++) {
        ...
        email   = this.emails[i],
        // emailId = t.blankNode();
        emailId = t.namedNode(`${QRM}id/vcard/${this.ownerId.bareUserId}/email/${i+1}`);
      ...
      for (let i = 0; i < this.phones.length; i++) {
        ...
        phone   = this.phones[i],
        // phoneId = t.blankNode();
        phoneId = t.namedNode(`${QRM}id/vcard/${this.ownerId.bareUserId}/phone/${i+1}`);
      ...
      if (this.photo) {
        if (this.photo.url) {
          ...
          const photoId = t.namedNode(`${QRM}id/vcard/${this.ownerId.bareUserId}/photo`);
          rdfDataset.add(
            t.quad(
              ownerId,
