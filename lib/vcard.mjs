// RDF
import { rdfTerm as t, IRI, iri,} from "../deps.mjs";
...
class Vcard { 
   ...
   * @param {{formattedName: string, vcardId: IRI | undefined, kind: string | undefined, name: Name | undefined, nickname: string | undefined, emails: Email[] | undefined, phones: Phone[] | undefined, photo: Photo | undefined, org: string | undefined}} vcard 
   * @throws {TypeError}
   */
  constructor(vcard){
    if (
      ...
      (vcard.vcardId && !(vcard.vcardId instanceof IRI)) ||
   ...
   * @returns {{formattedName: string, vcardId: IRI | undefined, kind: string | undefined, name: Name | undefined, nickname: string | undefined, emails: Email[] | undefined, phones: Phone[] | undefined, photo: Photo | undefined, org: string | undefined}} vcard 
   * @throws {TypeError}
   */
  static fromString(vcardText){ // TODO delegate some of the parsing to other classes
    try {
  ...
  /**
   * Writes the vCard to an RDF dataset. Vcards that don't have a vcardId are not written.
   * @param {object} rdfDataset
   * @throws {Error}
   */
  writeTo(rdfDataset){
    if (!this.vcardId) throw new Error("Vcard must have a vcardId");
    ...
    try {
      ...
      const 
      vcardId    = t.namedNode(`${this.vcardId}`);
      rdfDataset.add(
        t.quad(
          vcardId,
  ...
  static readFrom(rdfDataset){
    try {
      ...
      vcardIdsDataset = rdfDataset.match(null, a, vcardTypeTerm);

      // handle each vcard
      for (const vcardIdQuad of vcardIdsDataset) {
        const
        vcardIdTerm = vcardIdQuad.subject,
        vcardId = iri`${vcardIdTerm.value}`,
        vcardTypeDataset = rdfDataset.match(vcardIdTerm, a);
        ...
        // find formatted name
        let formattedName;
        const fnDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}fn`));
        ...
        // find nickname
        let nickname;
        const nicknameDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}nickname`));
        ...
        // find org
        let org;
        const orgDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}organization-name`));
        ...
        const familyNamesDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}family-name`));
        for (const familyNameQuad of familyNamesDataset) {
          familyNames.push(familyNameQuad.object.value.trim()); nameExists = true;
        }
        const givenNamesDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}given-name`));
        for (const givenNameQuad of givenNamesDataset) {
          givenNames.push(givenNameQuad.object.value.trim()); nameExists = true;
        }
        const additionalNamesDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}additional-name`));
        for (const additionalNamesQuad of additionalNamesDataset) {
          additionalNames.push(additionalNamesQuad.object.value.trim());
        }
        const honorificPrefixesDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}honorific-prefix`));
        ...
        const honorificSuffixesDataset = rdfDataset.match(vcardIdTerm, t.namedNode(`${VCARD}honorific-suffix`));
        ...
        res.push(new Vcard({
          formattedName, vcardId, kind, name, nickname, org,
          // emails, phones, photo, 
        }));
...
class IndividualVcard extends Vcard {
  ...
  static fromString(vcardText){
    ...
    return new IndividualVcard(vcardInput);
  }
  ...
  static readFrom(rdfDataset){
    ...
    .map(vcard => new IndividualVcard({
      formattedName: vcard.formattedName,
      vcardId      : vcard.vcardId,
      name         : vcard.name,
      kind         : vcard.kind,
...
class OrgVcard extends Vcard {
  ...
  static fromString(vcardText){
    ...
    if (vcardInput.kind) {
      ...
      return new OrgVcard(vcardInput);
    }
...
class GroupVcard extends Vcard {
  ...
  static fromString(vcardText){
    ...
    if (vcardInput.kind) {
      ...
      return new GroupVcard(vcardInput);
    }
