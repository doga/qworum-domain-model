...
class Persona {
  ...
  /**
   * Creates a persona object. The default role for the user and the group is
   * @param {{groupId: GroupId, userId: UserId, userVcard: IndividualVcard, groupVcard: GroupVcard, partnerGroupIds: GroupId[] | undefined}} persona 
   * @throws {TypeError}
   */
  constructor(persona) {
    ...
    if (!persona.partnerGroupIds.every(id => id instanceof GroupId && !id.equals(persona.groupId))) {
      throw new TypeError("Partner groups must be an array of GroupIds and must not contain the persona's group id");
    }
