 ...
 * 3. The role <https://vocab.site.example/id/role/unrestricted> is applicable to services that are hosted on site.example, subdomain.site.example, sub.subdomain.site.example etc. The special `vocab.` prefix is ignored (except if it's the domain owner's root domain, as in `vocab.com`). This is to allow service vendors to operate their own vocabulary servers for their developers and users.
 */
class Role {
  /** @type {RoleId} */
  roleId;
  ...
  constructor(role){
    ...
    this.domain      = new DnsDomain(role.roleId.hostname);
  }

  /**
   * 
   * @param {(string | DnsDomain | IRL | URL)} serviceDnsName contains the service's host name
   * @throws {TypeError}
   */
  isApplicableToQworumService(serviceDnsName){
    if ([IRL, URL].some(X => serviceDnsName instanceof X)) {
      if (!['http:', 'https:'].includes(serviceDnsName.protocol)) {
        throw new TypeError(`not an HTTP(S) URL/IRL`);
      }
      serviceDnsName = new DnsDomain(serviceDnsName.hostname);
    } else if (typeof serviceDnsName === 'string') {
      serviceDnsName = new DnsDomain(serviceDnsName);
    } else if (!(serviceDnsName instanceof DnsDomain)) {
      throw new TypeError(`not a dns name`);
    }

    // dnsName instanceof DnsDomain

    if (
      `${this.domain}` === 'vocab.qworum.net'
    ) return true;

    if (
      `${this.domain}`.startsWith('vocab.')
    ) {
      if (this.domain.isDirectTldSubdomain()) {
        return `${serviceDnsName}`.endsWith(`${this.domain}`);
      } else {
        return `${serviceDnsName}`.endsWith(`${this.domain}`.substring('vocab.'.length));
      }
    } else {
      return `${serviceDnsName}`.endsWith(`${this.domain}`);
    }
