...
class Roleset {
  ...
  /**
   * 
   * @param {(Role | IRL)} rootRole 
   * @returns {Role[]}
   * @throws {TypeError}
   */
  getDescendentsOf(rootRole){
    if(!(rootRole instanceof Role)) rootRole = this.findRole(rootRole);
    if(!rootRole)throw new TypeError('role not found in roleset');
    ...
    while (addedRoleToResults) {
      ...
      for (const role of this.roles) {
        if(alreadyMarkedAsADescendent(role))continue;
        if (
          // root role is direct parent?
          rootRole.roleId.equals(role.parentRoleId) ||

          descendentRoleIsParentOf(role)
        ) {
          res.push(role);
