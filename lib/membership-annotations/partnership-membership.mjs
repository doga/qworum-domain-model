// RDF
import { IRL, irl, rdfTerm as t, rdf } from "../../deps.mjs";
import { QRM, RDF } from "../util/rdf-prefixes.mjs";

import { 
  GroupId, MembershipId, PartnershipId, PartnershipMembershipId, 
  group_id, membership_id, partnership_id, partnership_membership_id,
} from "../id.mjs";
...
class PartnershipMembership {
   ...
   */
  get partnershipMembershipId(){return this.#partnershipMembershipId;}
   ...
   * @returns {IRL[]}
   */
  get roleIds(){return [...this.#roleIds];}
   ...
   * @param {{partnershipMembershipId: PartnershipMembershipId | undefined, partnershipId: PartnershipId, groupId: GroupId, roleIds: IRL[] | undefined}} partnershipMembership
   * @throws {TypeError}
   */
  constructor(partnershipMembership) {
    if (!(
      !partnershipMembership.partnershipMembershipId ||
      partnershipMembership.partnershipMembershipId instanceof PartnershipMembershipId
    )) {
      throw new TypeError('unexpected partnershipMembershipId');
    }

    if (!(partnershipMembership.partnershipId instanceof PartnershipId)) {
      throw new TypeError('unexpected partnershipId');
    }

    if (!(partnershipMembership.groupId instanceof GroupId)) {
      throw new TypeError('unexpected groupId');
    }

    if (!(
      !partnershipMembership.roleIds ||
      (partnershipMembership.roleIds instanceof Array && partnershipMembership.roleIds.every(r => r instanceof IRL))
    )) {
      throw new TypeError('unexpected roleIds');
    }

    if (!partnershipMembership.partnershipMembershipId) {
      partnershipMembership.partnershipMembershipId = PartnershipMembershipId.uuid();
    }

    if (!partnershipMembership.roleIds || partnershipMembership.roleIds.length === 0) {
      partnershipMembership.roleIds = [...defaultRoleset.roles.map(r => r.roleId)];
    }

    this.#partnershipMembershipId = partnershipMembership.partnershipMembershipId;
    this.#partnershipId           = partnershipMembership.partnershipId;
    this.#groupId                 = partnershipMembership.groupId;
    this.#roleIds                 = [...partnershipMembership.roleIds];
      ...
      for (const reifiedQuad of reificationsDs) {
        ...
        for (const annotationQuad of annotationsDs) {
          ...
          for (const roleIdQuad of roleIdsDs) {
            roleIds.push(irl`${roleIdQuad.object.value}`);
          }
