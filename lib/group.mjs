...
import { QRM, RDF, XSD } from "../lib/util/rdf-prefixes.mjs";
import { 
  OrgId, org_id, 
...
class Group {
   ...
   * 
   * If a partnership wishes to have this group as a member (indicated by fact that the partnership lists this group's id in its members list), then this group can accept the membership by pointing to the partnership with this field.
   * 
   ...
   * 
   * - The memberships manager hasn't explicity assigned R1 to the end-user, and hasn't assigned any other role from RS1 either. The memberships manager was probably unaware of the existence of the roleset. A decision must nevertheless must be taken, and that's exactly the purpose of this boolean flag: the end-user is implicitly assigned the role R1 only if this flag is set to `true`.
   * 
   * @returns {boolean}
   */
   ...
   * 
   * For personal groups, the `ownerIds` array must contain one `UserId`, and `isPersonalGroup` must be truthy and these fields must be omitted: `orgId`, `parentGroupId`, `subgroupsManagerIds`. If these fields are omitted for personal group, then they will be set to `ownerIds`: `partnershipsManagerIds`, `membershipsManagerIds`.
   * 
   * @param {{groupId: GroupId | undefined, isPersonalGroup: boolean | undefined, orgId: OrgId | undefined, parentGroupId: GroupId | undefined, partnershipId: PartnershipId | undefined, ownerIds: UserIdSet, subgroupsManagerIds: UserIdSet | undefined, partnershipsManagerIds: UserIdSet | undefined, membershipsManagerIds: UserIdSet | undefined, memberIds: UserIdSet | undefined, membersHaveAllRolesByDefault: boolean | undefined}} group
   * @throws {TypeError}
   */
  constructor(group) {
    if(!(
    ...
    if(!(
      group.membersHaveAllRolesByDefault === undefined ||
      typeof group.membersHaveAllRolesByDefault === 'boolean'
    ))
    ...
    this.#membersHaveAllRolesByDefault = (
      group.membersHaveAllRolesByDefault === undefined ? true : group.membersHaveAllRolesByDefault
    );
  }
  ...
  static readFrom(dataset){
    ...
    for (const idQuad of idsDataset) {
      ...
      const
      membersDs = dataset.match(idTerm, t.namedNode(`${QRM}member`)),
      membersHaveAllRolesByDefaultDs = dataset.match(idTerm, t.namedNode(`${QRM}membersHaveAllRolesByDefault`)),
      memberIds = new UserIdSet();
      ...
      // create group
      const group = new Group({groupId, isPersonalGroup, orgId, parentGroupId, partnershipId, ownerIds, subgroupsManagerIds, partnershipsManagerIds, membershipsManagerIds, memberIds, membersHaveAllRolesByDefault});
      res.push(group);
    }
