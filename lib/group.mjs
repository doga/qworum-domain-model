...
class Group {
   ...
   * collabManagerIds, membershipsManagerIds.
   * 
  ...
  constructor(group) {
    ...
    if (group.isPersonalGroup) {
      ...
      if (group.memberIds) {
        ...
        group.memberIds = group.ownerIds.filter(id => true);
      }
      if (group.orgId) {
      ...
      if (!group.collabManagerIds) {
        group.collabManagerIds = group.ownerIds.filter(id => true);
      }
      if (!group.membershipsManagerIds) {
        group.membershipsManagerIds = group.ownerIds.filter(id => true);
      }
    }
  ...
  static readFrom(dataset){
    ...
    for (const idQuad of idsDataset) {
      ...
      ownersDataset      = dataset.match(idTerm, t.namedNode(`${QRM}owner`)),
      subgroupsDataset   = dataset.match(idTerm, t.namedNode(`${QRM}subgroupsManager`)),
      collabDataset      = dataset.match(idTerm, t.namedNode(`${QRM}collabManager`)),
      membershipsDataset = dataset.match(idTerm, t.namedNode(`${QRM}membershipsManager`));
      ...
      const
      memberIds      = [],
      membersDataset = dataset.match(idTerm, t.namedNode(`${QRM}member`));
...
class PersonalGroup extends Group {
  /**
   * 
   * @param {{groupId: GroupId | undefined, collabId: CollabId | undefined, ownerIds: UserId[], collabManagerIds: UserId[] | undefined, membershipsManagerIds: UserId[] | undefined, memberIds: UserId[] | undefined}} group
   * @throws {TypeError}
   */
  constructor(group) {
    super({
