...
class Org {
  ...
  ownerIds;
  /**
   * Manager role: Groups manager. Can create root groups. Must not contain duplicate IDs (must be a set).
   * @type {UserId[]} 
   **/
  groupsManagerIds;
  /**
  ...
  get managerIds(){
    const res = [];
    [...this.groupsManagerIds, ...this.membershipsManagerIds]
    .forEach(
      id => {
        if(!res.find(id2 => id2.equals(id)))res.push(id);
      }
    );
    return res;
  }
  ...
  get participantIds(){
    const res = [];
    [...this.ownerIds, ...this.managerIds, ...this.memberIds]
    .forEach(
      id => {
        if(!res.find(id2 => id2.equals(id)))res.push(id);
      }
  ...
  /**
   * 
   * @param {(UserId | UserId[])} userIds
   * @returns {boolean}
   */
  hasOwners(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.ownerIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
  ...
  hasGroupsManagers(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.groupsManagerIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
  ...
  hasMembershipsManagers(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.membershipsManagerIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
  ...
  hasManagers(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.managerIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
  ...
  hasMembers(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.memberIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
  ...
  hasParticipants(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.participantIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
...
class Group {
  ...
  get managerIds(){
    const res = [];
    [...this.subgroupsManagerIds, ...this.partnershipsManagerIds, ...this.membershipsManagerIds]
    .forEach(
      id => {
        if(!res.find(id2 => id2.equals(id)))res.push(id);
      }
  ...
  hasSubgroupsManagers(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.subgroupsManagerIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
  ...
  hasPartnershipManagers(userIds) {
    if (!(userIds instanceof Array)) userIds = [userIds];
    for (const userId of userIds) {
      if(!this.partnershipsManagerIds.find(id => id.equals(userId))) return false;
    }
    return true;
  }
